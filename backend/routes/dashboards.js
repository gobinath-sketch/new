...
router.get('/business', authenticate, authorize('Business Head'), async (req, res) => {
  try {
    ...
    .select('opportunityId billingClient endClient finalGP tov gpPercent')
    .lean();
    
    const lowGPOpportunities = allOpps
      .filter(opp => {
        const gpPercent = (opp.finalGP / opp.tov) * 100;
        return gpPercent < 15;
      })
      .slice(0, 10);
    
    // Get current revenue target (Yearly for current year)
    const yearlyTarget = await RevenueTarget.findOne({ 
      year: currentYear, 
      period: 'Yearly' 
    }).sort({ createdAt: -1 });
    ...
    res.json({
      revenueTarget: yearlyTarget?.amount || 0,
      ...
      lowGPAlerts: lowGPOpportunities || [],
      aiInsight: aiInsight || ''
    });
  } catch (error) {
...
router.get('/director', authenticate, authorize('Director'), async (req, res) => {
  try {
    ...
    const allOppsForDirector = await Opportunity.find({
      tov: { $gt: 0 },
      finalGP: { $exists: true }
    })
    .select('opportunityId billingClient endClient finalGP tov gpPercent')
    ...
    const veryLowGPOpportunities = allOppsForDirector
      .filter(opp => {
        const gpPercent = (opp.finalGP / opp.tov) * 100;
        return gpPercent < 10;
      })
    ...
    res.json({
      ...
      recentOpportunities: allOpportunities || [],
      veryLowGPAlerts: [...(veryLowGPOpportunities || []), ...(veryLowGPPrograms || [])],
      aiInsight: aiInsight || ''
    });
  } catch (error) {
...
router.get('/sales-executive', authenticate, authorize('Sales Executive'), async (req, res) => {
  try {
    ...
    // Calculate metrics
    const revenueTarget = yearlyTarget?.amount || 0;
